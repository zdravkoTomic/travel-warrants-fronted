...
import {IVehicleType, IVehicleTypeModalData} from "./vehicleTypes";
...
export default function CatalogVehicleTypePage() {
    ...
    const toggleShowModal = (vehicleTypeId: number) => {
        fetch(
            ...
            .then(response => {
                    setModalData({
                        code: {
                            title: 'Kod prijevoznog sredstva',
                            value: response.code
                        },
                        name: {
                            title: 'Ime prijevoznog sredstva',
                            value: response.name
                        },
                        active: {
                            title: 'Aktivnost',
    ...
    const columns = [
        {
            ...
            cell: (props: any) => <Dropdown as={ButtonGroup}>
                ...
                <Dropdown.Menu>
                    <Dropdown.Item as={Link} to={`/vehicle_type_edit/${props.id}`}>Ažuriraj</Dropdown.Item>
                </Dropdown.Menu>
            </Dropdown>,
            ignoreRowClick: true,
            allowOverflow: true,
    ...
    const fetchData = () => {
        ...
        fetch(
            api.getUri() + `/vehicle-types?page=${encodeURIComponent(datatablePage)}&itemsperpage=${encodeURIComponent(perPage)}${order}`,
            {
                headers: {
            ...
            .then(response => response.json())
            .then(response => {
                setVehicleTypes(response['hydra:member'])
            ...
            {isAuthorized(['ROLE_ADMIN']) ? (
                <div>
                    ...
                    <DataTable
                        title={
                            <>
                                <h2 className="flex-display">Katalog - Prijevozna sredstva
                                    <Link className="add-new-record-btn" to="/vehicle_type_add">
